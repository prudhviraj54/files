...
class fraud_document_analytics:
  ...
  def paid_by_cash(self,read_res):
    ...
    read_res = read_res.withColumn('paid_by_cash', F.lit(False))

    def check_cash_payment(row):
    ...
    check_cash_payment_udf = F.udf(check_cash_payment,BooleanType())
    read_res = read_res.withColumn('paid_by_cash', check_cash_payment_udf(F.struct(*read_res.columns)))
